package wegotoaw.platform.scenario.board;

import wegotoaw.application.shared.models.board.NonObservedTileModel;
import wegotoaw.application.shared.models.board.ObservedTileModel;
import wegotoaw.application.shared.models.board.TileModel;
import wegotoaw.platform.engine.combat.CombatModifier;
import wegotoaw.platform.engine.knowledge.KnowledgeCategory;
import wegotoaw.platform.engine.knowledge.KnowledgeLevel;
...
import wegotoaw.platform.scenario.forces.*;
...
public final class Tile implements ModelProvider<TileModel> {
    ...
    public Tile(wegotoaw.data.wrappers.scenario.Cell c) {
//        x = c.getX();
//        y = c.getY();
        ...
        for (wegotoaw.data.wrappers.scenario.Terrain ct : c.getTerrain()) {
            wegotoaw.data.wrappers.scenario.TerrainType type = ct.getType();
        ...
        for (wegotoaw.data.wrappers.scenario.TerrainFeature feature : c.getFeature()) {
...
//        sb.append(owner).append('\n');
    ...
    private Map<MovementType, Integer> computeMinExitCosts() {
        ...
        for (Entry<MovementType, Integer> entry : minimumCosts.entrySet()) {
            MovementType mt = entry.getKey();
            int cost = MovementCost.IMPASSABLE;
            for (Entry<Direction, Tile> neighborEntry : neighbors.entrySet()) {
