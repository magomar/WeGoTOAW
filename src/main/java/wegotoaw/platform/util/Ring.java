...
/**
 * Implementation of a a generic linked circular list with point of interest
 *
 * @author Mario Gomez <margomez at dsic.upv.es>
 */
public class Ring<E> extends AbstractSequentialList<E>
        implements List<E>, Deque<E>, PointOfInterest, Cloneable, java.io.Serializable {

    transient int size = 0;
    /**
     * Pointer to first node. Invariant: (first == null && last == null) || (first.prev == null && first.item != null)
     */
    transient LinkedNode<E> first;
    /**
     * Pointer to last node. Invariant: (first == null && last == null) || (last.next == null && last.item != null)
     */
    transient LinkedNode<E> last;
    /**
     * Pointer to "Point of Interest" node. Invariant: (first == null && last == null) || (last.next == null &&
     * last.item != null)
     */
    transient LinkedNode<E> pointOfInterest;
    ...
    private void linkFirst(E e) {
        final LinkedNode<E> f = first;
        final LinkedNode<E> newNode = new LinkedNode<>(null, e, f);
        first = newNode;
        if (f == null) {
            last = newNode;
        } else {
     ...
     */
    void linkLast(E e) {
        final LinkedNode<E> l = last;
        final LinkedNode<E> newNode = new LinkedNode<>(l, e, null);
        last = newNode;
        if (l == null) {
            first = newNode;
            pointOfInterest = first;
        } else {
            l.next = newNode;
        }
        size++;
     ...
     */
    void linkBefore(E e, LinkedNode<E> succ) {
        // assert succ != null;
        final LinkedNode<E> pred = succ.prev;
        final LinkedNode<E> newNode = new LinkedNode<>(pred, e, succ);
        succ.prev = newNode;
        if (pred == null) {
            first = newNode;
        } else {
     ...
     */
    private E unlinkFirst(LinkedNode<E> f) {
        // assert f == first && f != null;
        final E element = f.item;
        final LinkedNode<E> next = f.next;
        f.item = null;
        f.next = null; // help GC
        first = next;
        if (next == null) {
     ...
     */
    private E unlinkLast(LinkedNode<E> l) {
        // assert l == last && l != null;
        final E element = l.item;
        final LinkedNode<E> prev = l.prev;
        l.item = null;
        l.prev = null; // help GC
        last = prev;
        if (prev == null) {
     ...
     */
    E unlink(LinkedNode<E> x) {
        // assert x != null;
        final E element = x.item;
        final LinkedNode<E> next = x.next;
        final LinkedNode<E> prev = x.prev;
        ...
        size--;
        modCount++;
        if (pointOfInterest == x) {
            pointOfInterest = (next == null ? prev : next);
        }
        return element;
    }

    /**
     * Returns the element in the current point of interest in this list
     *
     * @return the element in the current point of interest
     */
    @Override
    public E getPointOfInterest() {
        final LinkedNode<E> f = pointOfInterest;
        if (f == null) {
    ...
    /**
     * Advances the current point of interest to the next node. If the current point is the last element then moves the
     * point of interest to the first element, thus traversing the ring in a circular way
     */
    @Override
    public void next() {
        ...
        if (pointOfInterest == last) {
            pointOfInterest = first;
        } else {
            pointOfInterest = pointOfInterest.next;
        }
    }
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        ...
        for (Object o : a) {
            ...
            E e = (E) o;
            LinkedNode<E> newNode = new LinkedNode<>(pred, e, null);
            if (pred == null) {
                first = newNode;
                pointOfInterest = first;
            } else {
    ...
    public void clear() {
        ...
        first = last = pointOfInterest = null;
        size = 0;
        modCount++;
    }
...
//    }
     ...
     *
     * @return a shallow copy of this {@code Ring} instance
     */
    @Override
    public Object clone() {
        Ring<E> clone;
    ...
    /**
     * Saves the state of this {@code Ring} instance to a stream (that is, serializes it).
     *
    ...
    /**
     * Reconstitutes this {@code Ring} instance from a stream (that is, deserializes it).
     */
    @SuppressWarnings("unchecked")
